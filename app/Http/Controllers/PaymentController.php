    ...
    function editPayment()
    {
        ...
        if ($this->checkPassword($data['password'])) {
            return $this->handleEditPayment($data['customerId'], $data['payment_g'], $data['payment_t'], $data['day_rate'],
                $data['payment_id'], $data['payer_name'], $data['customer_name'], $data);
        } else {
            return [
                'status' => '2',
...
//    }
    ...
    function handleEditPayment($customerId, $payment_g, $payment_t, $day_rate, $payment_id, $payer_name, $customer_name, $data)
    {
        $paymentData = payment::where('id', $payment_id)->first();
        $paymentDataForEdit = payment::where('id', $payment_id)->update([
            'date' => $data['date'],
